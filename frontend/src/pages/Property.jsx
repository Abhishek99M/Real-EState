...
const Property = () => {
  ...
  return (
    <section
      className="max-padd-container !my-[99px]"
    ...
    >
      ...
      {/* CONTAINER */}
      <div className="xl:flex flex-row gap-8 items-start">
        {/* LEFT SIDE */}
        <div className="flex-1">
          <div className="flexStart gap-x-2"> 
            <FaLocationDot className="text-lime-400"/>
          ...
          <div className="flexBetween !py-1">
            <h5 className="bold-16 !my-1 text-lime-400">{data?.city}</h5>
            <div className="flex items-baseline gap-2 text-lime-400">
          ...
          <p className="mb-4 !mt-2 font-semibold text-gray-600">{data?.description}</p>
          <div className="fleBetween !pt-7">
            {bookings?.map((bookings) =>bookings.id).includes(id) ? (
              <>
                ...
                <p className="text-red-500 medium-15 ml-3 !mt-2">
                  You have already booked visit for  {bookings?.filter((bookings) => bookings?.id === id)[0].date}
                </p>
              </>
             ):(
              <Button 
