import React from "react";
import { FaCheck, FaStar } from "react-icons/fa";
import user1 from "../assets/testimonials/user1.png";
import user2 from "../assets/testimonials/user2.png";
import property1 from "../assets/img1.png";
import property2 from "../assets/img2.png";
import property3 from "../assets/img3.png";
import property4 from "../assets/img4.png";
const Testimonials = () => {
  return (
    <section className="max-padd-container bg-white !py-16 xl:pt-28">
      {/* CONTAINER */}
      <div
        className="grid grid-cols-1 lg:grid-cols-[2fr_5fr] gap-20 mb-16 rounded-2xl"
      >
        {/* LEFT SIDE */}
        <div className="flex items-start justify-between flex-col gap-10">
          <h2 className="h2">Customer feedback and experiences</h2>
          <div className="flex flex-col gap-1 bg-white !p-2 rounded ring-1 ring-slate-900/5">
            <div className="flex text-lime-400">
              <FaStar />
              <FaStar />
              <FaStar />
              <FaStar />
              <FaStar />
            </div>
            <div className="text-[14px] font-medium">
              Showing 1-2 out of <b>2k reviews</b>
            </div>
          </div>
        </div>
        {/* RIGHT SIDE */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-7">
          {/* TESTIMONIAL CARD 1*/}
          <div className="rounded-lg !p-4 bg-white ring-1 ring-slate-900/5">
            <div className="flexBetween">
              <div className="flexCenter gap-x-2">
                <img
                  src={user1}
                  alt="userImg"
                  height={44}
                  width={44}
                  className="rounded-full"
                />
                <h5 className="text-[14px] font-bold">John Doe</h5>
              </div>
              <div className="bg-lime-400 text-white rounded-full flexCenter gap-x-2 !p-1 !px-2 text-xs font-semibold">
                <FaCheck />
                Verified
              </div>
            </div>
            <hr className="h-[1px] w-full !my-2" />
            <div className="flex gap-x-1 text-lime-400 !mt-5 !mb-1 text-xs">
              <FaStar />
              <FaStar />
              <FaStar />
              <FaStar />
              <FaStar />
            </div>
            <h4 className="h4">High Quality</h4>
            <p className="text-gray-900 font-semibold">
              From start to finish, the entire home-buying process was seamless.
              The quality of service and attention to detail exceeded my
              expectations! I would highly recommend Landkit.
            </p>
            <div className="flex gap-x-1 !mt-5">
              <img
                src={property1}
                alt="property1Img"
                height={44}
                width={44}
                className="rounded aspect-square object-cover"
              />
              <img
                src={property2}
                alt="property1Img"
                height={44}
                width={44}
                className="rounded aspect-square object-cover"
              />
            </div>
          </div>
          {/* TESTIMONIAL CARD 2*/}
          <div
            className="rounded-lg !p-4 bg-white ring-1 ring-slate-900/5"
          >
            <div className="flexBetween">
              <div className="flexCenter gap-x-2">
                <img
                  src={user2}
                  alt="userImg"
                  height={44}
                  width={44}
                  className="rounded-full"
                />
                <h5 className="text-[14px] font-bold">Izabella Stress</h5>
              </div>
              <div className="bg-lime-400 text-white rounded-full flexCenter gap-x-2 !p-1 !px-2 text-xs font-semibold">
                <FaCheck />
                Verified
              </div>
            </div>
            <hr className="h-[1px] w-full !my-2" />
            <div className="flex gap-x-1 text-lime-400 !mt-5 !mb-1 text-xs">
              <FaStar />
              <FaStar />
              <FaStar />
              <FaStar />
              <FaStar />
            </div>
            <h4 className="h4">Modern Design</h4>
            <p className="text-gray-900 font-semibold">
              A perfect blend of elegance and functionality! The modern design
              and thoughtful layouts made my dream home a reality.
            </p>
            <div className="flex gap-x-1 !mt-5">
              <img
                src={property3}
                alt="property1Img"
                height={44}
                width={44}
                className="rounded aspect-square object-cover"
              />
              <img
                src={property4}
                alt="property1Img"
                height={44}
                width={44}
                className="rounded aspect-square object-cover"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Testimonials;
